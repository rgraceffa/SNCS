...
import sys
import argparse
from private_gpt.ui.xAgent import execute_remote_command
#, get_list_files, run_remote_script, run_remote_password_expiry_script, run_remote_enforce_pam_lockout
from private_gpt.ui.ollama_tools import generate_function_description, use_tools

agent_messages = [
    ('system',"Assistant is a Linux system administrator AI agent. This is an Linux system so only use Linux supported commands. Redirecting output using '>' is allowed. Please do so to create files. Assistant is designed to assist with Linux system commands, monitoring, and diagnostics. It can run system commands like `ifconfig`, `ip route show`, `netstat`, `ps -ef`, and `ls -l` on Linux hosts using the provided tools. When creating text files use echo and the > to redirect the output to the file. Do not use tee use echo > to redirect."),
    ('assistant', 'Hi there.'),
]

tools=[
       generate_function_description(execute_remote_command),
#       generate_function_description(get_list_files),
#       generate_function_description(run_remote_script),
#       generate_function_description(run_remote_password_expiry_script),
#       generate_function_description(run_remote_enforce_pam_lockout),
      ]
...
UI_TAB_TITLE = "SDU RO TA Chatbot"
...
class PrivateGptUi:
    ...
    def _chat(
        ...
        match mode:
            case Modes.RAG_MODE:
                ...
                userquery=all_messages[len(all_messages) - 1].content
                if userquery:
                  context_filter = None
                  ...
                  yield "Please provide an non empty request. Thanks."
            ...
            case Modes.AGENT_MODE:
                ...
                userquery= all_messages[len(all_messages) - 1].content
                if userquery:
                  agent_messages.append(("user", userquery))
                  try:
                  ...
                  if response['message']['content'] == "":
                    ...
                    result = use_tools(tools_calls, functions) # Call the agent
                    agent_messages.append(("tool", result))
                    response = query_model(
                           messages=agent_messages,
                           tools=tools,
                           )
                  else:
                    result = {}  # Initialize the variable again if the tools cannot be called 

                  agent_messages.append(("assistant", response['message']['content']))
                  #print(all_messages)
                  ...
                  if "status" in result:
                    ...
                    # Handle case where result doesn’t include "status"
                    print("[AGENT] ❌: agent not called")  
                    response['message']['content'] = (
                    "Agent Outcome\n\n"
                    f"Status: UNKNOWN\n"
                    f"Message: Unable to parse user input and detect the right agent\n"
                    f"Output: No output detected\n\n"
                    + response['message']['content']
                    ) 
                  
                  print("[LLM] ✅ : below the LLM response")
                  print(response)
    ...
    def _get_default_system_prompt(mode: Modes) -> str:
        ...
        match mode:
            ...
            # For agent mode use a dedicated system message
            case Modes.AGENT_MODE:
               p ="""You are a Linux system administrator AI agent operating on a Linux system or issuing commands remotely over SSH. Your goal is to output or execute only valid, safe Linux shell commands for diagnostics, monitoring, and system management.
...
"""
    ...
    def _get_default_mode_explanation(mode: Modes) -> str:
        match mode:
            ...
            case Modes.AGENT_MODE:
                return "Generate a request to action to the agents."
            case _:
                return ""
    ...
    def _build_ui_blocks(self) -> gr.Blocks:
        ...
        with gr.Blocks(
            ...
            with gr.Row():
                ...
                gr.HTML(
                        f"<div class='footer'><a class='footer-zylon-link' href='mailto:raimondo.graceffa@ericsson.com'>Maintained by ERAIGRA <img class='footer-zylon-ico' src='{f_base64}' alt=Zylon></a></div>"
                )
...
if __name__ == "__main__":
    ...
    _blocks.queue()
    _blocks.launch(debug=True, show_api=False)
